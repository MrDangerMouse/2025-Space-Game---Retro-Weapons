...
GPIO.setup(laser_missile_pin, GPIO.IN, pull_up_down=GPIO.PUD_UP)
GPIO.setup(key_special_pin, GPIO.IN, pull_up_down=GPIO.PUD_UP)
...
def run_loop(strip):
    ...
    for i in range(0, LED_COUNT):
        ...
        if GPIO.input(key_special_pin) == GPIO.HIGH:
            tasty = "craft"
        elif GPIO.input(key_special_pin) == GPIO.LOW:
            tasty = "special"
        
        if GPIO.input(laser_missile_pin) == GPIO.HIGH:
            nomnom = "laser"
        elif GPIO.input(laser_missile_pin) == GPIO.LOW:
            nomnom = "missile"
            
        if GPIO.input(fire_pin) == GPIO.LOW and led_on:
            print("SHOOT")
            space_api.shoot(nomnom)
